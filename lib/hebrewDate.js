...
// here.
//
// ESM module (run with `gjs -m`): extension.js is ESM and GNOME Shell's
// extension sandbox is not confirmed to expose the legacy `imports`
// global, so this file is ESM too rather than resting on that assumption.
// Only `gematria`, `hebrewDateParts` and `formatHebrewDate` are exported;
...
export function hebrewDateParts(date) {
...
export function formatHebrewDate(date, locale) {
