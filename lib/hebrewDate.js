...
// that read is stable across locales (see hebrewDateParts below).
...
const isHebrewLocale = locale =>
    locale === 'he' || locale.startsWith('he-') || locale.startsWith('he_');
...
function formatHebrewDate(date, locale) {
    if (isHebrewLocale(locale)) {
        ...
        return `${gematria(day)} ${HEBREW_MONTH_NAMES[month]} ${gematria(year % 1000)}`;
    }
